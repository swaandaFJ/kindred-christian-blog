...
function wireBookmarkButtons(root = document) {
  root.querySelectorAll("[data-bookmark-id]").forEach(btn => {
    ...
    btn.addEventListener("click", (e) => {
      ...
      syncBookmarkBtn(btn, id);
      showToast(nowActive ? "Saved to your bookmarks" : "Removed from bookmarks", "success");
    });
  });
}
...
function openGuestBookmarkPrompt(pendingId, triggerBtn) {
  ...
  overlay.style.cssText = "position:fixed;inset:0;background:rgba(34,31,29,.45);display:flex;align-items:center;justify-content:center;z-index:90;padding:16px;";
  overlay.innerHTML = `
